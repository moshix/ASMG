//LKEDCTL JOB CLASS=A,MSGCLASS=X,REGION=4096K
//*
//* DELETE ANY OLD TARGET LIBRARY
//*
//* STEP01   EXEC PGM=IEFBR14
//* SYSPRINT DD  SYSOUT=*
//* SYSUT2   DD DISP=(OLD,DELETE,DELETE),DSN=CBT593.ASMG27A.LKEDIN
//*
//* ALLOCATE TARGET LIBRARY AND LOAD WITH LINK CONTROL STATEMENTS
//*
//STEP02   EXEC PGM=IEBUPDTE,PARM=NEW
//SYSPRINT DD  SYSOUT=*
//SYSUT2   DD DISP=(NEW,CATLG,DELETE),DSN=CBT593.ASMG27A.LKEDIN,
//             DCB=(RECFM=FB,LRECL=80,BLKSIZE=800),
//             SPACE=(CYL,(2,1,2),RLSE),
//             UNIT=3330,VOL=SER=PSTCAD
//SYSIN DD *
./ ADD NAME=LINKALL
         NAME      LINKALL(R)
./  ADD  NAME=ASMGASM
         INCLUDE   MODLIB(ASMGASM)
         INCLUDE   MODLIB(ASMGBUFF)
         ENTRY     ASMGASM
         ALIAS     ASMG
         NAME      ASMGASM(R)
./  ADD  NAME=ASMGFPP
         INCLUDE   MODLIB(ASMGFPP)
         INCLUDE   MODLIB(ASMGFD)
         INCLUDE   MODLIB(ASMGFEX)
         ENTRY     ASMGFPP
         NAME      ASMGFPP(R)
./  ADD  NAME=ASMGF1
         INCLUDE   MODLIB(ASMGF1)
         ENTRY     ASMGF1
         NAME      ASMGF1(R)
./  ADD  NAME=ASMGF2
         INCLUDE   MODLIB(ASMGF2)
         INCLUDE   MODLIB(ASMGF2A)
         ENTRY     ASMGF2
         NAME      ASMGF2(R)
./  ADD  NAME=ASMGF3
         INCLUDE   MODLIB(ASMGF3)
         ENTRY     ASMGF3
         NAME      ASMGF3(R)
./  ADD  NAME=ASMGF7
         INCLUDE   MODLIB(ASMGF7C)
         INCLUDE   MODLIB(ASMGF7D)
         INCLUDE   MODLIB(ASMGF7E)
         INCLUDE   MODLIB(ASMGF7G)
         INCLUDE   MODLIB(ASMGF7I)
         INCLUDE   MODLIB(ASMGF7L)
         INCLUDE   MODLIB(ASMGF7N)
         INCLUDE   MODLIB(ASMGF7S)
         INCLUDE   MODLIB(ASMGF7V)
         INCLUDE   MODLIB(ASMGF7X)
         INCLUDE   MODLIB(ASMGFI)
         ENTRY     ASMGF7
         NAME      ASMGF7(R)
./  ADD  NAME=ASMGF8
         INCLUDE   MODLIB(ASMGF8A)
         INCLUDE   MODLIB(ASMGF8C)
         INCLUDE   MODLIB(ASMGF8D)
         INCLUDE   MODLIB(ASMGF8I)
         INCLUDE   MODLIB(ASMGF8L)
         INCLUDE   MODLIB(ASMGF8M)
         INCLUDE   MODLIB(ASMGF8N)
         INCLUDE   MODLIB(ASMGF8P)
         INCLUDE   MODLIB(ASMGF8S)
         INCLUDE   MODLIB(ASMGF8V)
         ENTRY     ASMGF8
         NAME      ASMGF8(R)
./  ADD  NAME=ASMGIS00
         INCLUDE  MODLIB(ASMGIS00)
         ENTRY     ASMGIS00
         NAME      ASMGIS00(R)
./  ADD  NAME=ASMGIS01
         INCLUDE  MODLIB(ASMGIS01)
         ENTRY     ASMGIS01
         NAME      ASMGIS01(R)
./  ADD  NAME=ASMGIS02
         INCLUDE  MODLIB(ASMGIS02)
         ENTRY     ASMGIS02
         NAME      ASMGIS02(R)
./  ADD  NAME=ASMGIS09
         INCLUDE  MODLIB(ASMGIS09)
         ENTRY     ASMGIS09
         NAME      ASMGIS09(R)
./  ADD  NAME=ASMGIS11
         INCLUDE  MODLIB(ASMGIS11)
         ENTRY     ASMGIS11
         NAME      ASMGIS11(R)
./  ADD  NAME=ASMGIS20
         INCLUDE  MODLIB(ASMGIS20)
         ENTRY     ASMGIS20
         NAME      ASMGIS20(R)
./  ADD  NAME=ASMGIS44
         INCLUDE  MODLIB(ASMGIS44)
         ENTRY     ASMGIS44
         NAME      ASMGIS44(R)
./  ADD  NAME=ASMGIS60
         INCLUDE   MODLIB(ASMGIS60)
         ENTRY     ASMGIS60
         NAME      ASMGIS60(R)
./  ADD  NAME=ASMGIS67
         INCLUDE  MODLIB(ASMGIS67)
         ENTRY     ASMGIS67
         NAME      ASMGIS67(R)
./  ADD  NAME=ASMGIS70
         INCLUDE   MODLIB(ASMGIS70)
         ENTRY     ASMGIS70
         ALIAS     ASMGIS01
         NAME      ASMGIS70(R)
./  ADD  NAME=ASMGIS71
         INCLUDE   MODLIB(ASMGIS71)
         ENTRY     ASMGIS71
         NAME      ASMGIS71(R)
./  ADD  NAME=ASMGMACP
         INCLUDE   MODLIB(ASMGMACP)
         ENTRY     ASMGMACP
         ALIAS     ASMGMAC7    <-ALIAS WAS ASMGF7
         NAME      ASMGMACP(R)
./  ADD  NAME=ASMGRTA
         INCLUDE   MODLIB(ASMGRTA)
         ENTRY     ASMGRTA
         NAME      ASMGRTA(R)
./  ADD  NAME=ASMGUP
         INCLUDE   MODLIB(ASMGUP)
         ENTRY     ASMGUP
         NAME      ASMGUP(R)
./  ADD  NAME=ASMGWYL
         INCLUDE   MODLIB(ASMGWYL)
         INCLUDE   MODLIB(ASMGBUFF)
         ENTRY     ASMGASM
         NAME      ASMGWYL(R)
/*
//
